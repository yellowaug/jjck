...
namespace socketServer
{
    ...
    public class SocketSQLAction : IConnectionDb,IRevertDbAction
    {
    ...
    public class RunProList
    {
        public void RunPro()
        {
            IConnectionDb connectionDb = new SocketSQLAction();
            var connect = connectionDb.InitConnection();
            IRevertDbAction revertDb = new SocketSQLAction();
            //string[] socketbackdblist = { "aspnetdb", "JJ_Communication", "JJ_Sale", "JJ_System", "JJlinshi" };
            string[] socketbackdblist = {"Account", "Book", "EFtest"};
            SocketFolderAction createPath = new SocketFolderAction();
            var revertPath=createPath.CreateFolder(@"D:\SocketDbFile");
            Console.WriteLine("读取备份文件的路径{0}",revertPath);
            var reverrtShellList=revertDb.GetDbrevert(connect, revertPath, socketbackdblist);
            foreach (var itemShell in reverrtShellList)
            {
                revertDb.AutoRevert(itemShell, connect);
                Console.WriteLine("备份的文件名{0}",itemShell.PathAndFileName);
            }
            connectionDb.Closeconnection(connect);
        }
        public void RunDeletePro()
        {
            SocketFolderAction createPath = new SocketFolderAction();
            var deletePath = createPath.CreateFolder(@"D:\SocketDbFile");
            SocketFolderAction deleteFolder = new SocketFolderAction();
            deleteFolder.DeleteFolder(deletePath);
        }
    }
}
